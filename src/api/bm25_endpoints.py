...
from .bm25_models import (
    ...
    BM25HealthResponse, BM25SearchRequest, BM25SearchResponse, EpisodeSearchResult,
...
async def bm25_chat(
    ...
    try:
        ...
        # Format sources with BM25 scores
        formatted_sources = []
        for source in result.get('sources', []):
            formatted_sources.append(BM25Source(**source))
        ...
        for method_name, method_data in comparison['methods'].items():
            if 'error' in method_data:
                ...
                methods[method_name] = SearchMethodResult(
                    ...
                    episodes_referenced=method_data['episodes_referenced'],
                    top_episodes=method_data['top_episodes']
                )
...
async def bm25_search_episodes(
    ...
    try:
        ...
        for episode in episodes:
            formatted_episodes.append(EpisodeSearchResult(
        ...
        return BM25SearchResponse(
            ...
            total_found=len(formatted_episodes),
            processing_time=processing_time
        )
    ...
    try:
        ...
        try:
            ...
            original_rag_result = RAGChainResult(
                ...
                episodes_referenced=original_result.get('episode_references', []),
        ...
        try:
            ...
            bm25_rag_result = RAGChainResult(
                ...
                episodes_referenced=bm25_result.get('episode_references', []),
        ...
        analysis = {
            ...
            'source_comparison': {
                ...
                'unique_original_episodes': set(original_rag_result.episodes_referenced) - set(bm25_rag_result.episodes_referenced),
                'unique_bm25_episodes': set(bm25_rag_result.episodes_referenced) - set(original_rag_result.episodes_referenced),
                'common_episodes': list(set(original_rag_result.episodes_referenced) & set(bm25_rag_result.episodes_referenced))
            }
        }
        ...
        if both_successful := (original_rag_result.success and bm25_rag_result.success):
            if len(bm25_rag_result.episodes_referenced) > len(original_rag_result.episodes_referenced):
                recommendation = "BM25 RAG found more diverse episode sources"
            elif bm25_rag_result.processing_time < original_rag_result.processing_time:
                recommendation = "BM25 RAG was faster"
            elif len(analysis['source_comparison']['common_episodes']) > 0:
                recommendation = "Both chains found similar relevant episodes"
            else:
