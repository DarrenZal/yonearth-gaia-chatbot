...
async def qa_hybrid(request: Request, body: HybridQARequest = Body(...)) -> HybridQAResponse:
    start = time.time()
    try:
        # Try BM25+vector+graph fused via hybrid retriever FIRST (respects book+episode mixing)
        used_bm25 = False
        sources: List[HybridSource] = []
        ...
        # Optionally add standalone GraphRetriever sources (secondary), mainly for transparency and backfill.
        # In graph_mode='off' we skip this; in 'evidence_only' and 'full' we still
        # allow graph to contribute extra sources/evidence without affecting the
        # BM25HybridRetriever's backbone guarantees.
        g_res = None
        graph_mode = getattr(settings, "graph_mode", "full")
        if settings.enable_graph_retrieval and graph_mode != "off":
            try:
        ...
        note = None
        try:
            retrieved_docs = list(bm_docs)
            if g_res:
                # Attach graph chunks for added context without overriding BM25 fusion
        ...
        # If graph disabled, annotate note for transparency
        if not settings.enable_graph_retrieval:
            note = (note + " ") if note else ""
            note = (note or "") + "Graph retrieval disabled (feature flag)."
