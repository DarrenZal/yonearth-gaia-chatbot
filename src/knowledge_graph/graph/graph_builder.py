...
class GraphBuilder:
    ...
    def __init__(
        ...
        type_strict_matching: bool = False
    ):
        """
        ...
        Args:
            ...
            neo4j_client: Neo4j client instance (optional for JSON-only export)
            similarity_threshold: Fuzzy matching threshold (0-100). Default 90.
            validator: EntityMergeValidator instance for semantic validation
        ...
        self.name_canonical_map: Dict[str, str] = {}
    ...
    def load_extractions(self) -> Dict[str, Any]:
        ...
        """
        extraction_files = list(self.extraction_dir.glob("*_extraction.json"))
            ...
            for entity_id, entity in entity_list:
                ...
                for other_id, other_entity in entity_list:
                    ...
                    if entity_name == other_name or similarity >= self.similarity_threshold:
                        # If validator is provided, check if merge is allowed
                        if self.validator:
                            can_merge, reason = self.validator.can_merge(
                                entity, other_entity, log_rejection=False
                            )
                            if not can_merge:
                                logger.debug(
                                    f"Validator rejected merge: '{entity['name']}' + "
                                    f"'{other_entity['name']}' - {reason}"
                                )
                                continue

                        # If type_strict_matching, enforce type compatibility
                        if self.type_strict_matching and entity['type'] != other_entity['type']:
                            logger.debug(
                                f"Type mismatch: '{entity['name']}' ({entity['type']}) + "
                                f"'{other_entity['name']}' ({other_entity['type']})"
                            )
                            continue
                ...
                self.name_canonical_map[merged_entity['name'].lower()] = merged_entity['name']

                # Create mapping for old IDs to canonical ID
                for old_id, _ in similar_entities:
                    entity_id_mapping[old_id] = canonical_id
                    # Map original names to canonical name for later relationship rewrite
                    original_name = self.entities[old_id]['name']
                    self.name_canonical_map[original_name.lower()] = merged_entity['name']
    ...
    def deduplicate_relationships(self) -> Dict[str, Any]:
        ...
        # Rewrite relationship endpoints to canonical names after deduplication
        if self.name_canonical_map:
    ...
    def export_unified_json(self, output_path: str):
        """
        Export unified knowledge graph to JSON file.

        This creates a unified.json file compatible with the existing format
        used by graph_endpoints.py and visualization tools.

        Args:
            output_path: Path to output JSON file
        """
        logger.info(f"Exporting unified knowledge graph to {output_path}")

        # Build entities dict (name -> entity_data)
        entities_dict = {}
        for entity_id, entity in self.entities.items():
            name = entity['name']
            entities_dict[name] = {
                'type': entity['type'],
                'description': entity.get('description', ''),
                'sources': entity.get('metadata', {}).get('episodes', []),
        ...
        for rel in self.relationships:
            rel_dict = {
                ...
                'type': rel['relationship_type'],  # Add both predicate and type
                'description': rel.get('description', ''),
                'source_type': self._get_entity_type(rel['source_entity']),
                'target_type': self._get_entity_type(rel['target_entity']),
        ...
        logger.info(
            f"Exported {len(entities_dict)} entities and "
            f"{len(relationships_list)} relationships to {output_path}"
        )

        # Log validation statistics if validator was used
        if self.validator:
