#!/usr/bin/env python3
"""
Knowledge Graph Extraction V11.1 - Fixed Modular Architecture

🎯 V11.1 FIXES (from V11):
1. ✅ Fixed module interface: Convert dicts → ModuleRelationship objects
2. ✅ Reduced chunk size: 800 → 600 words (prevent token limit)
3. ✅ Added retry logic: Auto-split and retry failed chunks

🐛 V11 BUGS FIXED:
- Bug #1: Modules crashed with "'dict' object has no attribute 'source'"
  → Fixed by creating ModuleRelationship wrapper class
- Bug #2: 2 chunks hit 16K token output limit (lost ~9 pages)
  → Fixed by reducing chunk size and adding retry with split
...
Based on V11 but with critical bug fixes.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'kg_extraction_book_v11_1_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
OUTPUT_DIR = PLAYBOOK_DIR / "output" / "v11_1"
...
class ModuleRelationship:
    """
    ✨ V11.1 FIX: Relationship format compatible with postprocessing modules.
    ...
    def pattern_prior(self) -> float:
        """Default pattern prior (not used in V11.1 but ListSplitter expects it)"""
        return 0.5

    @property
    def claim_uid(self) -> Optional[str]:
        """Default claim_uid (not used in V11.1 but ListSplitter expects it)"""
        return None

    @property
    def extraction_metadata(self) -> Dict[str, Any]:
        """Default extraction metadata (not used in V11.1 but ListSplitter expects it)"""
        return {}
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'ModuleRelationship':
        ...
        ✨ V11.1.1 FIX: Filters out property-only keys that ListSplitter might add
...
def create_chunks(
    ...
    ✨ V11.1 FIX: Reduced default chunk_size from 800 → 600 words
...
def evaluate_pass2(
    ...
    ✨ V11.1 FIX: Returns ModuleRelationship objects (not dicts)
        ...
        try:
            ...
            for eval_result in result.evaluations:
                ...
                if 0 <= cand_idx < len(batch):
                    ...
                    # ✨ V11.1 FIX: Create ModuleRelationship (not dict)
...
def postprocess_pass2_5(
    ...
    ✨ V11.1 FIX: Pass ModuleRelationship objects (not dicts) to pipeline.
    Modules can now access rel.source, rel.evidence, rel.flags, etc.
    ...
    # ✨ V11.1 FIX: Pass objects directly (not dicts)
    # Modules expect objects with attributes, not dicts
...
def extract_knowledge_graph_v11_1(
    book_title: str,
    pdf_path: Path,
    ...
    """
    V11.1 knowledge graph extraction pipeline with bug fixes.

    ✨ V11.1 FIXES:
    1. ModuleRelationship objects → modules can access attributes
    2. Reduced chunk size 800 → 600 words → prevent token limit
    3. Retry logic → auto-split chunks that hit token limit
    ...
    5. Pass 2.5: Modular postprocessing with proper objects
    6. Return results with full metadata
    """
    logger.info(f"🚀 Starting V11.1 extraction: {book_title}")
    ...
    results = {
        'metadata': {
            ...
            'document_metadata': document_metadata,
            'fixes_applied': [
                'ModuleRelationship objects for proper module interface',
                'Reduced chunk size 800 → 600 words',
                'Auto-retry with split on token limit'
            ]
        },
        'extraction_stats': {
    ...
    logger.info("")
    logger.info("📊 FINAL V11.1 RESULTS:")
    ...
    logger.info(f"  - ✨ V11.1 Pass 2.5 final (modular): {len(final_relationships)}")
...
def main():
    """Extract knowledge graph from Soil Stewardship Handbook with V11.1 fixed system"""
    logger.info("="*80)
    logger.info("🚀 V11.1 KNOWLEDGE GRAPH EXTRACTION - FIXED MODULAR ARCHITECTURE")
    logger.info("="*80)
    logger.info("")
    logger.info("✨ V11.1 FIXES:")
    logger.info("  1. ✅ Fixed module interface: ModuleRelationship objects (not dicts)")
    logger.info("  2. ✅ Reduced chunk size: 800 → 600 words (prevent token limit)")
    logger.info("  3. ✅ Added retry logic: Auto-split chunks on token limit")
    logger.info("")
    logger.info("🐛 V11 BUGS FIXED:")
    logger.info("  - ALL 10 modules crashed with 'dict has no attribute' errors")
    logger.info("  - 2 chunks hit 16K token limit (lost ~9 pages of relationships)")
    logger.info("")
    logger.info("GOAL: Proper modular postprocessing with pronoun resolution,")
    logger.info("      metaphor labeling, dedication parsing, and classification")
    logger.info("")
    ...
    run_id = f"book_soil_handbook_v11_1_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # Extract with V11.1 fixed system
    results = extract_knowledge_graph_v11_1(
        book_title=book_title,
        pdf_path=pdf_path,
        document_metadata=document_metadata,
        run_id=run_id,
    ...
    # Save results
    output_path = OUTPUT_DIR / f"{book_title.replace(' ', '_').lower()}_v11_1.json"
    ...
    logger.info("")
    logger.info("="*80)
    logger.info("✨ V11.1 EXTRACTION COMPLETE")
    logger.info("="*80)
    logger.info(f"⏱️  Total time: {total_time/60:.1f} minutes")
    logger.info(f"📁 Results saved to: {output_path}")
    logger.info("")
    logger.info("NEXT STEPS:")
    logger.info("1. Run KG Reflector on V11.1 to verify fixes worked")
    logger.info("2. Compare V11.1 vs V11 (broken) vs V10 vs V9 quality metrics")
    logger.info("3. Verify modules ran successfully (check module_flags in output)")
    logger.info("4. If successful, V11.1 becomes baseline for next ACE cycle")
    logger.info("="*80)
