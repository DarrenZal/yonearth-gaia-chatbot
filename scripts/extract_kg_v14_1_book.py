#!/usr/bin/env python3
"""
Knowledge Graph Extraction V14.1 - Path to A+ Grade

🎯 V14.1 PHILOSOPHY: "Extract comprehensively, evaluate strictly"

V14.1 FIXES V14.0 REGRESSION:
- V14.0 PROBLEM: Pass 1 too restrictive → extracted only 596 candidates (vs V13.1's 861)
- ROOT CAUSE DISCOVERED: V13.1 used V12's checkpoint! V12's 23KB prompt extracted 861 candidates
- V14.1 TESTED: V10 simple prompt (7.5KB) extracted only 401 candidates (WORSE!) ❌
- V14.1 SOLUTION: Use V12's proven complex prompt (23KB) + 15 postprocessing modules

🎯 V14.1 ENHANCEMENTS (from V14.0 B+):
1. ✅ USING V12 Prompt: Proven complex prompt that extracted 861 candidates (NOT V10!)
2. ✅ NEW SemanticDeduplicator Module: Remove 25 redundant 'is-a' relationships (embedding-based)
3. ✅ FIXED SemanticDeduplicator Bug: Local variable reference error fixed
4. ✅ Pipeline Metrics: Comprehensive logging of Pass 1/2/2.5 filtering breakdown
5. ✅ Kept from V14.0: Enhanced Pass 2, MetadataFilter, ConfidenceFilter, Predicate Normalizer V1.4

🐛 V14.0 ISSUES FIXED:
- Issue #1: Pass 1 too restrictive (596 vs 861) → USE V12's proven 23KB prompt
- Issue #2: Redundant 'is-a' relationships (25 issues, 4.2%) → NEW SemanticDeduplicator
- Issue #3: No pipeline visibility → Added comprehensive metrics logging
- ROOT CAUSE: V14 prompt had different issues than V12's prompt

📊 EMPIRICAL DATA (TEST RESULTS):
- V12 prompt (23KB, complex) → V13.1 extracted 861 candidates → A- grade (8.6%) ✅ BEST
- V14 prompt (27KB, complex) → V14.0 extracted 596 candidates → B+ grade (10.78%) ❌
- V10 prompt (7.5KB, simple) → V14.1 TEST extracted 401 candidates (31% WORSE!) ❌❌
- **LESSON: V12's complexity worked, simpler ≠ better**

🏗️ Architecture:
- Pass 1: V12's PROVEN complex prompt (comprehensive extraction + quality guidance)
- Pass 2: V14's Dual-signal evaluation (kept - it's good!)
- Pass 2.5: 15 postprocessing modules (14 from V14 + NEW SemanticDeduplicator)

Target: A or A+ grade (~2.9% issue rate, ~870 rels, ~25 issues)
Expected improvement: 65 → 25 issues (-62%), 10.78% → 2.9% (-73%)
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'kg_extraction_book_v14_1_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
# ✨ Load V14.1 prompts from files
logger.info("Loading V14.1 prompts from files...")
PASS1_PROMPT_FILE = PROMPTS_DIR / "pass1_extraction_v14_1.txt"
PASS2_PROMPT_FILE = PROMPTS_DIR / "pass2_evaluation_v14.txt"  # V14.1 uses same Pass 2 as V14

if not PASS1_PROMPT_FILE.exists():
    logger.warning(f"⚠️  V14.1 Pass 1 prompt not found, falling back to V14")
    PASS1_PROMPT_FILE = PROMPTS_DIR / "pass1_extraction_v14.txt"

if not PASS2_PROMPT_FILE.exists():
    logger.warning(f"⚠️  V14.1 Pass 2 prompt not found, falling back to V13.1")
...
logger.info(f"✅ V14.1 prompts loaded successfully from {PASS1_PROMPT_FILE.name} and {PASS2_PROMPT_FILE.name}")
...
def postprocess_pass2_5(
    ...
    ✨ V14.1 ENHANCEMENTS: 15 modules including new SemanticDeduplicator (V14: 14 modules)
    ...
    """
    logger.info("🔧 PASS 2.5: Running V14.1 modular postprocessing pipeline (15 modules)...")

    # Get book pipeline (15 modules with V14.1 enhancements)
...
def extract_knowledge_graph_v14_1(
    book_title: str,
    pdf_path: Path,
    ...
    """
    V14.1 knowledge graph extraction pipeline - Path to A+ Grade.

    🎯 V14.1 PHILOSOPHY: "Extract comprehensively, evaluate strictly"

    ✨ V14.1 KEY DECISION: REVERT to V10's Simple Prompt
    - V10 prompt (7.5KB) → V13.1 extracted 861 candidates → A- grade ✅
    - V14 prompt (27KB) → V14.0 extracted 596 candidates → B+ grade ❌
    - Over-engineering the prompt backfired - simplicity + strong postprocessing wins!

    ✨ V14.1 ENHANCEMENTS (from V14.0 B+):
    1. REVERTED to V10 Prompt: Simple, permissive extraction (proven to get 861 candidates)
    2. NEW SemanticDeduplicator: Remove 25 redundant 'is-a' relationships using embeddings
    3. Pipeline Metrics: Comprehensive logging of Pass 1/2/2.5 filtering breakdown
    4. Kept from V14.0: Enhanced Pass 2, MetadataFilter, ConfidenceFilter, Predicate Normalizer V1.4
    5. Trust the Pipeline: 15 postprocessing modules handle quality

    Architecture:
    1. Extract text from PDF
    2. Create chunks with overlap (600 words)
    3. Pass 1: V10's SIMPLE permissive prompt (861 candidates expected)
    4. Pass 2: V14's Dual-signal evaluation (kept - it's good!)
    5. Pass 2.5: 15 modular postprocessing modules (14 from V14 + SemanticDeduplicator)
    6. Return results with full metadata + pipeline metrics

    Target: A or A+ grade (~2.9% issue rate, ~870 rels, ~25 issues)
    Expected improvement: 65 → 25 issues (-62%), 10.78% → 2.9% (-73%)
    """
    logger.info(f"🚀 Starting V14.1 extraction: {book_title}")
    ...
    # Step 5: Pass 2.5 - Modular postprocessing with V14.1 enhancements
    context = ProcessingContext(
        content_type='book',
    ...
    results = {
        'metadata': {
            ...
            'enhancements_applied': [
                'V14.1: KEY DECISION - Reverted to V10 simple prompt (proven to extract 861 candidates)',
                'V14.1: V14 over-engineering backfired - 27KB prompt extracted only 596 vs V10\'s 861',
                'V14.1: NEW SemanticDeduplicator - removes 25 redundant "is-a" relationships using embeddings',
                'V14.1: Pipeline metrics logging - comprehensive Pass 1/2/2.5 filtering breakdown',
                'V14.1: Trust the pipeline - 15 postprocessing modules handle quality, not prompt restrictions',
                'V14.1: Kept from V14.0 - Enhanced Pass 2 with improved claim classification',
                'V14.1: Kept from V14.0 - MetadataFilter module (4-layer detection)',
                'V14.1: Kept from V14.0 - ConfidenceFilter module (flag-specific thresholds)',
                'V14.1: Kept from V14.0 - Predicate Normalizer V1.4 (tense norm, semantic validation)',
                'V14.1: Target grade: A or A+ (~2.9% issue rate, ~870 rels, ~25 issues)'
            ]
        },
        'extraction_stats': {
    ...
    logger.info("")
    logger.info("📊 FINAL V14.1 RESULTS:")
    ...
    logger.info(f"  - ✨ V14.1 Pass 2.5 final: {len(final_relationships)}")
...
def main():
    """Extract knowledge graph from Soil Stewardship Handbook with V14.1 - Path to A+ Grade"""
    logger.info("="*80)
    logger.info("🚀 V14.1 KNOWLEDGE GRAPH EXTRACTION - PATH TO A+ GRADE")
    logger.info("="*80)
    logger.info("")
    logger.info("🎯 V14.1 PHILOSOPHY: 'Extract comprehensively, evaluate strictly'")
    logger.info("")
    logger.info("📊 EMPIRICAL TEST RESULTS DRIVING V14.1:")
    logger.info("  - V12 prompt (23KB, complex) → V13.1 extracted 861 candidates → A- grade (8.6%) ✅ BEST")
    logger.info("  - V14 prompt (27KB, complex) → V14.0 extracted 596 candidates → B+ grade (10.78%) ❌")
    logger.info("  - V10 prompt (7.5KB, simple) → V14.1 TEST extracted 401 candidates (31% worse!) ❌❌")
    logger.info("  - ROOT CAUSE: V13.1 used V12's checkpoint! Simpler ≠ better, V12's complexity worked!")
    logger.info("")
    logger.info("✨ V14.1 KEY DECISION: USE V12'S PROVEN COMPLEX PROMPT")
    logger.info("  1. ✅ Use V12's proven complex prompt (23KB, comprehensive + quality guidance)")
    logger.info("  2. ✅ NEW SemanticDeduplicator: Remove 25 redundant 'is-a' relationships using embeddings")
    logger.info("  3. ✅ FIXED SemanticDeduplicator bug: Local variable reference error")
    logger.info("  4. ✅ Pipeline Metrics: Comprehensive logging of Pass 1/2/2.5 filtering breakdown")
    logger.info("  5. ✅ Kept from V14.0: Enhanced Pass 2, MetadataFilter, ConfidenceFilter, Predicate Normalizer V1.4")
    logger.info("")
    logger.info("🎯 EXPECTED IMPROVEMENTS:")
    logger.info("  - V14.0 Grade: B+ (10.78% issue rate, 65 issues, 596 candidates)")
    logger.info("  - V14.1 Target: A or A+ (~2.9% issue rate, ~25 issues, ~870 candidates)")
    logger.info("  - Expected improvement: 65 → 25 issues (-62%), 596 → 870 candidates (+46%)")
    logger.info("")
    ...
    run_id = f"book_soil_handbook_v14_1_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # Extract with V14.1 - Path to A+ Grade
    results = extract_knowledge_graph_v14_1(
        book_title=book_title,
        pdf_path=pdf_path,
        document_metadata=document_metadata,
        run_id=run_id,
    ...
    # Save results
    output_path = OUTPUT_DIR / f"{book_title.replace(' ', '_').lower()}_v14_1.json"
    ...
    logger.info("")
    logger.info("="*80)
    logger.info("✨ V14.1 EXTRACTION COMPLETE")
    logger.info("="*80)
    logger.info(f"⏱️  Total time: {total_time/60:.1f} minutes")
    logger.info(f"📁 Results saved to: {output_path}")
    logger.info("")
    logger.info("NEXT STEPS:")
    logger.info("1. Run KG Reflector on V14.1 to verify quality improvements")
    logger.info("2. Compare V14.1 vs V14.0 (B+) quality metrics")
    logger.info("3. Validate target: ~870 relationships, ~25 issues, ~2.9% rate (A+)")
    logger.info("4. If grade reaches A or A+, V14.1 becomes new stable release")
    logger.info("="*80)
