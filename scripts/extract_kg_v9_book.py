#!/usr/bin/env python3
"""
Knowledge Graph Extraction v9 - REFLECTOR-GUIDED CRITICAL FIXES

✨ V9 CRITICAL FIXES (From V8 Reflector Analysis):
✅ CRITICAL: FIXED PraiseQuoteDetector - now identifies SPEAKER vs SUBJECT (fixes 3 errors)
✅ CRITICAL: NEW p_true threshold filtering - filters low-confidence relationships (fixes 8 errors)
✅ HIGH: ENHANCED pronoun resolver - possessive pronoun resolution improved (fixes 12 errors)
✅ HIGH: ENHANCED Pass 1 prompt - constraints, conciseness, few-shot examples (prevents ~24 errors)
✅ HIGH: ENHANCED Pass 2 prompt - calibrated knowledge plausibility scoring (prevents ~9 errors)

GOAL: Reduce quality issues from 8.35% (V8) to <5% (V9) ✅ TARGET: 3.2-3.7%

V8 REGRESSION ANALYSIS:
- V8 had 91 issues (8.35%) vs V7's 62 issues (6.71%) - regression of +29 issues
- Root causes: Incomplete praise quote fix, no p_true threshold, over-extraction, permissive prompts
- V9 addresses all root causes with 5 targeted fixes

Architecture: V8 base + 5 Reflector-guided critical fixes → V9 Production System

V9 inherits all V8 + V7 + V6 improvements and adds:
- Praise quote SPEAKER attribution (FIXED V9)
- Quality threshold filtering p_true >= 0.5 (NEW V9)
- Enhanced possessive pronoun patterns (ENHANCED V9)
- Stricter Pass 1 extraction prompt with examples (NEW V9)
- Calibrated Pass 2 evaluation with scoring guide (NEW V9)
- Praise quote detection in front matter (V8)
- Context-aware vague entity replacement (V8)
- 'and' conjunction list splitting (V8)
- Semantic predicate validation (V8)
- Dedication detection (V8)
- Metaphor normalization (V8)
- POS-tagging for list splitting (V6)
- Multi-pass pronoun resolution (V7, V8, ENHANCED V9)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'kg_extraction_book_v9_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
OUTPUT_DIR = PLAYBOOK_DIR / "output" / "v9"
...
# ============================================================================
# PASS 1 & PASS 2 (from V5 - unchanged)
# ============================================================================

BOOK_EXTRACTION_PROMPT = """Extract SPECIFIC, CONCRETE relationships from this text.
...
❌ Overly abstract concepts ("spiritual flourishing", "the crossroads", "the answer")
❌ Metaphors and figurative language ("veritable Eden", "road-map")
❌ Philosophical definitions ("what it means to be human")
❌ Overly verbose entities (full sentences as entities)
❌ Subjective opinions without factual grounding
...
✅ GOOD: "biochar production methods"
❌ BAD: "the various methods and techniques used for biochar production"
...
✅ GOOD: (Aaron William Perry, hails from, Slovenia)
✅ GOOD: (Slovenians, lived in, Slovenia)

**Why**: Always resolve pronouns to specific entities before extracting.
...
✅ SKIP THIS: This is a philosophical opinion, not a factual relationship.

**Why**: Extract facts, not philosophical definitions or subjective opinions.

### Example 3: Concrete vs. Abstract ⚖️
...
✅ BETTER: (human wellbeing, depends on, environmental stewardship)

**Why**: If you must extract abstract concepts, use concrete terms. But prefer skipping overly abstract statements.

### Example 4: Entity Conciseness ✂️
...
✅ GOOD: (soil education, benefits, families)

**Why**: Use concise noun phrases. Avoid gerunds and verbose descriptions.

### Example 5: Metaphors ❌
**Text**: "Our land has remained a veritable Eden through the ages."
❌ BAD: (Our land, is, a veritable Eden)
✅ SKIP THIS: "Veritable Eden" is metaphorical, not a factual description.

**Why**: Avoid extracting metaphors and figurative language.
...
### 2. DISCOURSE GRAPH
- **Claims**: Factual assertions, verifiable arguments
- **Evidence**: Facts, data, observations that support or oppose claims
- **Questions**: Specific inquiries, problems, challenges posed
- Relationships:
...
**EXCLUDE** from discourse graph:
- Philosophical musings ("what it means to be X")
- Abstract opinions without evidence
...
**Focus on**: Concrete, repeatable processes with specific entities.
...
**Focus on**: Verifiable causal relationships, not philosophical claims.

### 5. DEFINITIONS & DESCRIPTIONS
- **Identity**: X is-a Y, X is defined as Y
- **Properties**: X has Y, X contains Y, X exhibits Y
- **Comparison**: X is similar to Y, X differs from Y

**EXCLUDE**:
- Metaphorical identities ("X is a compass for Y")
- Philosophical definitions ("X is what it means to be Y")
...
- **source**: Concise source entity (2-5 words, no pronouns)
- **relationship**: Relationship type (supports, causes, is-a, produces, etc.)
- **target**: Concise target entity (2-5 words, no pronouns)
...
# ⭐ V9 ENHANCEMENT: Calibrated dual-signal evaluation with explicit scoring guidelines
