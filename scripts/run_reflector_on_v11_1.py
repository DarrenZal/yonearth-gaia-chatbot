#!/usr/bin/env python3
"""
Run KG Reflector on V11.1 extraction results

Analyzes V11.1 quality with modular postprocessing system
...
def main():
    print("="*80)
    print("🤔 RUNNING KG REFLECTOR ON V11.1 OUTPUT (MODULAR POSTPROCESSING)")
    print("="*80)
    print()
    ...
    # Load V11.1 results
    print("📂 Loading V11.1 extraction results...")
    ...
    extraction_metadata = {
        'version': v11_1_data.get('version', 'v11.1'),
        'book_title': v11_1_data.get('book_title', 'Soil Stewardship Handbook'),
        'date': v11_1_data.get('timestamp', v11_1_data['metadata'].get('extraction_timestamp', 'N/A')),
        'total_relationships': len(v11_1_data['relationships']),
        'high_confidence_count': sum(1 for r in v11_1_data['relationships'] if r.get('p_true', 0) >= 0.75),
        'extraction_stats': v11_1_data.get('extraction_stats', {}),
        'module_flags': v11_1_data.get('extraction_stats', {}).get('module_flags', {})
    }
    ...
    print()
    print("="*80)
    print("🤔 ANALYZING V11.1 QUALITY WITH CLAUDE SONNET 4.5...")
    print("="*80)
    print("This will take 2-3 minutes for comprehensive analysis...")
    print()
    print("V11.1 Modular Postprocessing Approach:")
    print("  ✅ V11 bugs fixed: ModuleRelationship wrapper for proper module interface")
    print("  ✅ Chunk size reduced from 800 → 600 words to prevent token limits")
    print("  ✅ Automatic retry with chunk splitting on token limit errors")
    print("  ✅ 10/10 postprocessing modules executed successfully")
    print("  ✅ ListSplitter reprocessing: 989 → 1,180 relationships (+191)")
    print()
    ...
    analysis = reflector.analyze_kg_extraction(
        relationships=v11_1_data['relationships'],
    ...
    # Compare V9 → V10 → V11.1
    print("="*80)
    print("📊 PROGRESSIVE COMPARISON: V9 → V10 → V11.1")
    print("="*80)
    print()
    ...
    print(f"V11.1 Results (Modular Postprocessing with Bug Fixes):")
    ...
    if v9_analysis and v10_analysis:
        ...
        print(f"  V10 → V11.1: {v11_1_issues - v10_issues:+d} issues ({v11_1_rate - v10_rate:+.1f}% error rate)")
        print(f"  V9 → V11.1: {v11_1_issues - v9_issues:+d} issues ({v11_1_rate - v9_rate:+.1f}% error rate)")
        print()

        v9_rel_count = len(v9_data['relationships'])
        v10_rel_count = len(v10_data['relationships'])
        v11_1_rel_count = len(v11_1_data['relationships'])
        ...
        print(f"  V10 → V11.1: {v11_1_rel_count - v10_rel_count:+d} relationships ({(v11_1_rel_count/v10_rel_count - 1)*100:+.1f}%)")
        print(f"  V9 → V11.1: {v11_1_rel_count - v9_rel_count:+d} relationships ({(v11_1_rel_count/v9_rel_count - 1)*100:+.1f}%)")
        print()
    ...
    if target_met:
        ...
        print()
        print("1. ✅ V11.1 achieves production quality (A++ grade)")
        print("2. 📊 Document V11.1 results in V11_1_RESULTS.md")
        print("3. 🚀 Use V11.1 as baseline for future ACE cycles")
        print("4. 📚 Consider applying to full corpus (172 episodes + 3 books)")
    else:
        print("📈 Continue ACE cycle:")
        print()
        print("1. Review V11.1 Reflector analysis")
        ...
        print()
        print("Key V11.1 Achievements:")
        print("  ✅ Fixed module interface mismatch (dicts → objects)")
        print("  ✅ Fixed token limit failures (600-word chunks + retry)")
        print("  ✅ Fixed ListSplitter compatibility (@property aliases)")
        print("  ✅ All 10 modules executing successfully")
