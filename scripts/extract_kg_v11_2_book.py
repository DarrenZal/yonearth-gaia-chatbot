#!/usr/bin/env python3
"""
Knowledge Graph Extraction V11.2 - Fixed Modular Architecture

🎯 V11.2 FIXES (from V11):
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'kg_extraction_book_v11_2_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
OUTPUT_DIR = PLAYBOOK_DIR / "output" / "v11_2"
...
class ModuleRelationship:
    """
    ✨ V11.2 FIX: Relationship format compatible with postprocessing modules.
        ...
        try:
            ...
            for eval_result in result.evaluations:
                ...
                if 0 <= cand_idx < len(batch):
                    ...
                    # ✨ V11.2 FIX: Create ModuleRelationship (not dict)
...
def extract_knowledge_graph_v11_2(
    book_title: str,
    pdf_path: Path,
    ...
    """
    V11.2 knowledge graph extraction pipeline with bug fixes.
    ...
    """
    logger.info(f"🚀 Starting V11.2 extraction: {book_title}")
    ...
    logger.info("")
    logger.info("📊 FINAL V11.2 RESULTS:")
    ...
    logger.info(f"  - ✨ V11.2 Pass 2.5 final (modular): {len(final_relationships)}")
...
def main():
    """Extract knowledge graph from Soil Stewardship Handbook with V11.2 fixed system"""
    logger.info("="*80)
    logger.info("🚀 V11.2 KNOWLEDGE GRAPH EXTRACTION - FIXED MODULAR ARCHITECTURE")
    logger.info("="*80)
    logger.info("")
    logger.info("✨ V11.2 FIXES:")
    ...
    run_id = f"book_soil_handbook_v11_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # Extract with V11.2 fixed system
    results = extract_knowledge_graph_v11_2(
        book_title=book_title,
        pdf_path=pdf_path,
        document_metadata=document_metadata,
        run_id=run_id,
    ...
    # Save results
    output_path = OUTPUT_DIR / f"{book_title.replace(' ', '_').lower()}_v11_2.json"
    ...
    logger.info("")
    logger.info("="*80)
    logger.info("✨ V11.2 EXTRACTION COMPLETE")
    logger.info("="*80)
    logger.info(f"⏱️  Total time: {total_time/60:.1f} minutes")
    logger.info(f"📁 Results saved to: {output_path}")
    logger.info("")
    logger.info("NEXT STEPS:")
    logger.info("1. Run KG Reflector on V11.2 to verify fixes worked")
    logger.info("2. Compare V11.2 vs V11 (broken) vs V10 vs V9 quality metrics")
    logger.info("3. Verify modules ran successfully (check module_flags in output)")
    logger.info("4. If successful, V11.2 becomes baseline for next ACE cycle")
    logger.info("="*80)
