#!/usr/bin/env python3
"""
Knowledge Graph Extraction v8 - CURATOR-GENERATED ACE CYCLE 1 PRODUCTION SYSTEM

✨ V8 NEW FEATURES (Generated by KG Curator from V7 Reflector Analysis):
✅ CRITICAL: NEW PraiseQuoteDetector module - detects/corrects praise quotes (fixes 4 errors)
✅ CRITICAL: ENHANCED pronoun resolver - possessive pronouns + 5-sentence window (fixes 18 errors)
✅ HIGH: ENHANCED vague entity detector - context-aware replacement (fixes 8 errors)
✅ HIGH: ENHANCED list splitter - handles 'and' conjunctions (fixes 4 errors)
✅ HIGH: PROMPT ENHANCEMENT - philosophical statement filter (filters 6 errors)
✅ MEDIUM: ENHANCED predicate normalizer - semantic validation (fixes 2 errors)
✅ MEDIUM: ENHANCED bibliographic parser - dedication detection (fixes 1 error)
✅ MEDIUM: ENHANCED figurative language filter - metaphor normalization (fixes 3 errors)

GOAL: Reduce quality issues from 6.71% (V7) to <3% (V8) ✅ TARGET: 2.7%

Architecture: V7 base + 8 Curator-generated enhancements → V8 Production System

V8 inherits all V7 + V6 improvements:
- Praise quote detection in front matter (NEW V8)
- Possessive pronoun resolution (ENHANCED V8)
- Context-aware vague entity replacement (ENHANCED V8)
- 'and' conjunction list splitting (ENHANCED V8)
- Philosophical statement filtering (NEW V8)
- Semantic predicate validation (NEW V8)
- Dedication detection (NEW V8)
- Metaphor normalization (NEW V8)
...
- Large pronoun resolution window (V6, ENHANCED V8)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'kg_extraction_book_v8_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
OUTPUT_DIR = PLAYBOOK_DIR / "output" / "v8"
...
def _default_extraction_metadata():
    return {
        ...
        "prompt_version": "v8_curator_ace",
        "extractor_version": "2025.10.12_v8",
        "content_type": "book",
        "run_id": None,
        "extracted_at": None,
        "batch_id": None
...
class PraiseQuoteDetector:
    ...
    FIXES: 4 reversed authorship errors (0.43%)
    """
    ...
    def __init__(self):
        ...
        self.person_name_pattern = r'—([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)'
...
# ✨ V8 ENHANCEMENT (Curator Change #007): Philosophical statement filter
...
3. ✨ V8 NEW: FACTUAL CONCRETENESS (Curator Change #007):
   - Does the relationship convey concrete, verifiable information?
   - Downweight abstract/philosophical statements

### ✨ V8 NEW: DOWNWEIGHT ABSTRACT/PHILOSOPHICAL STATEMENTS ###
...
**Examples of what to REJECT (low confidence):**
...
**Examples of what to ACCEPT (high confidence):**
