#!/usr/bin/env python3
"""
Incremental Knowledge Graph Extraction V14.3.5 (Isolated)

Compartmentalized variant to avoid interfering with other agents.
- Uses an isolated pipeline configuration with safer list splitting and
  explicit front-matter direction corrections.
"""
...
from src.knowledge_graph.postprocessing.pipelines.book_pipeline import get_book_pipeline_v1435
...
def update_status(status_path: Path, section: str, extraction_file: str, logger):
    if status_path.exists():
        ...
        status = {'version': 'v14_3_5', 'book': 'our_biggest_deal', 'last_updated': datetime.now().isoformat(), 'sections': {}}
    section_info = status.get('sections', {}).get(section, {})
    iterations = section_info.get('iterations', 0) + 1
    status['sections'][section] = {'status': 'in_progress', 'current_extraction': extraction_file, 'iterations': iterations, 'last_updated': datetime.now().isoformat()}
...
def generate_execution_manifest(section: str, args: argparse.Namespace, prompts: Dict[str, Path], logger) -> Dict[str, Any]:
    ...
    manifest = {
        ...
        'packages': {'openai': packages.get('openai', 'unknown'), 'pydantic': packages.get('pydantic', 'unknown'), 'pdfplumber': packages.get('pdfplumber', 'unknown')},
        'script': {'path': str(Path(__file__).relative_to(Path.cwd())), 'args': vars(args)},
        'prompts': prompt_checksums,
        'model_config': {'pass1_model': 'gpt-4o-2024-08-06', 'pass2_model': 'gpt-4o-2024-08-06', 'temperature': 0.0, 'max_tokens': 16384}
    }
...
def evaluate_pass2(candidates: List[ExtractedRelationship], client: OpenAI, evaluation_prompt: str, batch_size: int = 25, model: str = "gpt-4o-2024-08-06", logger=None) -> List[ModuleRelationship]:
    ...
    for i in range(0, len(candidates), batch_size):
        ...
        for idx, rel in enumerate(batch):
            batch_json.append({'candidate_uid': f"cand_{i+idx}", 'source': rel.source, 'relationship': rel.relationship, 'target': rel.target, 'context': rel.context, 'page': rel.page})
        try:
            response = client.beta.chat.completions.parse(
                model=model,
...
def postprocess_pass2_5(relationships: List[ModuleRelationship], context: ProcessingContext, logger) -> Tuple[List[ModuleRelationship], Dict[str, Any]]:
    logger.info("🔧 PASS 2.5: Running v14.3.5 isolated modular postprocessing pipeline...")
    pipeline = get_book_pipeline_v1435()
...
def extract_section(args: argparse.Namespace):
    ...
    logger.info("="*80)
    logger.info("🚀 INCREMENTAL KNOWLEDGE GRAPH EXTRACTION V14.3.5 (Isolated)")
    logger.info("="*80)
    ...
    # Prompts (fallback search)
    pass1_candidates = [
        PROMPTS_DIR / "pass1_extraction_v14_3_5.txt",
    ...
    pass2_candidates = [
        PROMPTS_DIR / "pass2_evaluation_v14_3_5.txt",
    ...
    pass1_prompt_file = next((p for p in pass1_candidates if p.exists()), PROMPTS_DIR / "pass1_extraction_v14_3_1.txt")
    pass2_prompt_file = next((p for p in pass2_candidates if p.exists()), PROMPTS_DIR / "pass2_evaluation_v13_1.txt")
    ...
    _, pages_with_text = extract_pages_from_pdf(pdf_path, args.pages, logger)
    logger.info("")
    ...
    for i, chunk in enumerate(chunks):
        ...
        candidates, _ = extract_pass1(chunk, client, extraction_prompt, logger=logger)
    ...
    # Pass 2.5
    logger.info("🔧 PASS 2.5: Modular postprocessing (isolated)...")
    document_metadata = {'author': args.author if hasattr(args, 'author') else 'Unknown', 'title': args.book.replace('_', ' ').title(), 'section': args.section, 'pages': args.pages}
    context = ProcessingContext(content_type='book', document_metadata=document_metadata, pages_with_text=pages_with_text, run_id=f"{args.section}_{timestamp}", extraction_version=args.version)
    final_relationships, pp_stats = postprocess_pass2_5(evaluated, context, logger)
    logger.info("")
    ...
    output_path = output_dir / "chapters" / output_filename
    output_path.parent.mkdir(parents=True, exist_ok=True)
    manifest['duration_seconds'] = elapsed
    manifest['output_file'] = str(output_path.relative_to(Path(__file__).parent.parent))
    results = {
        'metadata': {'book': args.book, 'section': args.section, 'pages': args.pages, 'extraction_version': args.version, 'timestamp': timestamp, 'extraction_date': datetime.now().isoformat(), 'elapsed_seconds': elapsed, 'document_metadata': document_metadata},
        'extraction_stats': {'pass1_candidates': len(all_candidates), 'pass2_evaluated': len(evaluated), 'pass2_5_final': len(final_relationships)},
    ...
    manifest_path = output_dir / "manifests" / f"{args.section}_execution_{timestamp}.json"
    manifest_path.parent.mkdir(parents=True, exist_ok=True)
    with open(manifest_path, 'w') as f:
        json.dump(manifest, f, indent=2)
    update_status(status_path, args.section, output_filename, logger)
    logger.info("="*80)
    logger.info("✅ INCREMENTAL EXTRACTION COMPLETE (Isolated v14.3.5)")
    logger.info("="*80)
    logger.info(f"  Section: {args.section}")
...
def main():
    parser = argparse.ArgumentParser(description="Incremental Knowledge Graph Extraction V14.3.5 (Isolated)", formatter_class=argparse.RawDescriptionHelpFormatter, epilog="""
Examples:
  python3 scripts/extract_kg_v14_3_5_incremental.py \
    ...
    parser.add_argument('--version', default='v14_3_5', help='Extraction version (default: v14_3_5)')
