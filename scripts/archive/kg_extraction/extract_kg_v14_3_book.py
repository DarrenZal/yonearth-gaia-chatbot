#!/usr/bin/env python3
"""
Knowledge Graph Extraction V14.3 - Full V13.1 Baseline Restoration

🎯 V14.3 STRATEGY: ESTABLISH STABLE A- BASELINE

This is an EXACT REPLICA of V13.1's proven configuration to establish a stable baseline.

**Configuration**:
- ✅ V12 Pass 1 Prompt: Comprehensive extraction with simple predicates
- ✅ V13.1 Pass 2 Prompt: Dual-signal evaluation
- ✅ V13.1 Pass 2.5: 12-module pipeline (proven A- grade)

**Why V14.3**:
- V14.0 regressed from A- to B+ (10.78% issue rate)
- V14.1 failed with C+ (16.7% issue rate) - V12 prompt extracts low-quality content
- V14.2 failed catastrophically with C+ (27.3% issue rate) - predicate fragmentation

**Root Cause Identified**:
V14's Pass 1 prompt extracts VERBOSE predicates ("is key for", "is made manifest by") that V13.1's
PredicateNormalizer wasn't designed to handle. This caused 100 predicate fragmentation issues (19.3%).

**V14.3 Goal**: Restore A- baseline (8.6% issue rate, ~873 relationships) as foundation for incremental improvements.

**Next Steps After V14.3**:
- Phase 2 (V14.4): Enhance PredicateNormalizer for A- to A grade
- Phase 3 (V14.5): Add philosophical claim filtering for A grade
- Phase 4 (V14.6): Add MetadataFilter for A to A+ grade
- Phase 5 (V15): Add ConfidenceFilter for A+ grade (<5% issue rate)

🏗️ Architecture:
- Pass 1: V12 prompt (comprehensive, simple predicates)
- Pass 2: V13.1 prompt (dual-signal evaluation)
- Pass 2.5: 12-module pipeline (PraiseQuote → Deduplicator)
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'kg_extraction_book_v14_3_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
OUTPUT_DIR = PLAYBOOK_DIR / "output" / "v14_3"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# ✨ Load V14.3 prompts from files (EXACT V13.1 configuration)
logger.info("Loading V14.3 prompts (V13.1 baseline)...")
PASS1_PROMPT_FILE = PROMPTS_DIR / "pass1_extraction_v12.txt"  # ✅ V12 Pass 1: simple predicates
PASS2_PROMPT_FILE = PROMPTS_DIR / "pass2_evaluation_v13_1.txt"  # ✅ V13.1 Pass 2: dual-signal

if not PASS1_PROMPT_FILE.exists():
    raise FileNotFoundError(f"V12 Pass 1 prompt not found: {PASS1_PROMPT_FILE}")
...
logger.info(f"✅ V14.3 prompts loaded successfully from {PASS1_PROMPT_FILE.name} and {PASS2_PROMPT_FILE.name}")
...
def postprocess_pass2_5(
    ...
    ✨ V14.3: Uses V13.1's 12-module pipeline (version='v13')
    ...
    # ✨ V14.3: Explicitly use V13.1's 12-module pipeline
...
def extract_knowledge_graph_v14_3(
    book_title: str,
    pdf_path: Path,
    ...
    results = {
        'metadata': {
            ...
            'document_metadata': document_metadata,
            'configuration': {
                'pass1_prompt': 'pass1_extraction_v12.txt',
                'pass2_prompt': 'pass2_evaluation_v13_1.txt',
                'pass2_5_pipeline': 'v13 (12 modules)',
                'strategy': 'Full V13.1 baseline restoration'
            },
            'fixes_applied': [
...
def main():
    ...
    run_id = f"book_soil_handbook_v14_3_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    results = extract_knowledge_graph_v14_3(
        book_title=book_title,
        pdf_path=pdf_path,
        document_metadata=document_metadata,
        run_id=run_id,
    ...
    # Save results
    output_path = OUTPUT_DIR / f"{book_title.replace(' ', '_').lower()}_v14_3.json"
