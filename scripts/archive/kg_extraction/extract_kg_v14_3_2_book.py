#!/usr/bin/env python3
"""
Knowledge Graph Extraction V14.3.2 - Resolve-Then-Block Workflow + Bibliographic Fixes

🎯 V14.3.2 STRATEGY: RESOLVE VAGUE ENTITIES BEFORE BLOCKING

**Goal**: Achieve A+ grade (1-2% issue rate) through intelligent entity resolution and targeted fixes.

**Configuration**:
- ✅ V14.3.1 Pass 1 Prompt: FACTUAL-ONLY extraction (same as V14.3.1)
- ✅ V13.1 Pass 2 Prompt: Dual-signal evaluation (same as V14.3.1)
- ✅ V14.3.2 Pass 2.5: 13-module pipeline with RESOLVE-THEN-BLOCK workflow

**Why V14.3.2**:
- V14.3.1 achieved A grade (5.3% actionable issues, excluding philosophical content)
- User philosophy: "err on the side of overextracting than underextracting"
- User guidance: "resolve stuff like 'the amount' to '10 lbs', and if we can't, then block it"

**Three High-Impact Improvements**:

1. **BibliographicCitationParser Enhancements** (v1.4.0, -9 issues):
   - Fix reversed authorship: Book → authored → Author becomes Author → authored → Book
   - Fix dedication targets: "Book Title to Name" becomes "Name"
   - Expected impact: -1 CRITICAL + -8 HIGH issues

2. **Resolve-Then-Block Workflow** (-7 to -10 issues):
   ...
   - Resolves: "unknown" publisher → actual publisher, "community activities" → specific activity

3. **Enhanced Vague Entity Patterns**:
   - Added specific patterns from V14.3.1 analysis: "unknown", "community activities", "personal life-hacks"
   - Blocks only entities that couldn't be resolved by ContextEnricher

**Expected Result**: 5.3% → 1-2% issue rate (A → A+ grade)

🏗️ Architecture:
- Pass 1: V14.3.1 prompt (factual-only, specific entities, resolved pronouns)
- Pass 2: V13.1 prompt (dual-signal evaluation)
- Pass 2.5: V14.3.2 pipeline (resolve-then-block + enhanced bibliographic parsing)
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'kg_extraction_book_v14_3_2_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
OUTPUT_DIR = PLAYBOOK_DIR / "output" / "v14_3_2"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# ✨ Load V14.3.1 prompts (V14.3.2 uses same prompts, different postprocessing)
logger.info("Loading V14.3.1 prompts (V14.3.2 uses same prompts + enhanced postprocessing)...")
...
logger.info("   V14.3.2 uses V14.3.1 prompts + enhanced postprocessing (resolve-then-block workflow)")
...
def postprocess_pass2_5(
    ...
    ✨ V14.3.2: Uses V14.3.2's 13-module pipeline (version='v14_3_2')
    ...
    """
    logger.info("🔧 PASS 2.5: Running V14.3.2 modular postprocessing pipeline (13 modules)...")

    # ✨ V14.3.2: Explicitly use V14.3.2's 13-module pipeline
    pipeline = get_book_pipeline(version='v14_3_2')
...
def extract_knowledge_graph_v14_3_2(
    book_title: str,
    pdf_path: Path,
    ...
    results = {
        'metadata': {
            ...
            'configuration': {
                ...
                'pass2_5_pipeline': 'v14_3_2 (13 modules: resolve-then-block workflow)',
                'strategy': 'Resolve vague entities before blocking + bibliographic fixes'
            },
            'fixes_applied': [
                'V14.3.2 FIX 1: BibliographicCitationParser v1.4.0 (authorship + dedication)',
                'V14.3.2 FIX 2: ContextEnricher v1.2.0 (priority 30, enhanced resolution)',
                'V14.3.2 FIX 3: VagueEntityBlocker v1.1.0 (priority 85, flag-based blocking)',
                'V14.3.2 WORKFLOW: Resolve-then-block (ContextEnricher → VagueEntityBlocker)',
                'Expected improvement: 5.3% → 1-2% issue rate (A → A+ grade)'
            ]
        },
        'extraction_stats': {
...
def main():
    ...
    run_id = f"book_soil_handbook_v14_3_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    results = extract_knowledge_graph_v14_3_2(
        book_title=book_title,
        pdf_path=pdf_path,
        document_metadata=document_metadata,
        run_id=run_id,
    ...
    # Save results
    output_path = OUTPUT_DIR / f"{book_title.replace(' ', '_').lower()}_v14_3_2.json"
