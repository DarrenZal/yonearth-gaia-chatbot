#!/usr/bin/env python3
"""
Knowledge Graph Extraction v3.2.2 - IMPROVED Book Implementation

COMPLETE implementation of KG_MASTER_GUIDE_V3.md for books:
✅ Type validation with SHACL-lite domain/range checking
✅ Geographic validation with 3-tier checking (admin → population → distance)
✅ Pattern priors from existing graph analysis
✅ Full validator integration from src/knowledge_graph/validators.py
✅ Character-level evidence spans
✅ Complete dual-signal evaluation
...
def _default_extraction_metadata():
    ...
    return {
        ...
        "prompt_version": "v3.2.2_book_improved",
        "extractor_version": "2025.10.11",
        "content_type": "book",
        "run_id": None,
        "extracted_at": None,
        "batch_id": None
...
# ============================================================================
# ENTITY PRESENCE VALIDATION (✨ NEW - ENSURES ENTITIES IN EVIDENCE)
# ============================================================================

def validate_entity_presence(rel: ProductionRelationship) -> tuple[bool, str]:
    """
    ✨ NEW: Validate that entities actually appear in evidence text
    Returns: (is_valid, reason_if_invalid)
    """
    evidence_lower = rel.evidence_text.lower()
    source_lower = rel.source.lower()
    target_lower = rel.target.lower()

    # Check exact match first
    source_present = source_lower in evidence_lower
    target_present = target_lower in evidence_lower

    if source_present and target_present:
        return True, ""

    # Check word-by-word for multi-word entities (70% threshold)
    if not source_present:
        source_words = source_lower.split()
        if len(source_words) > 1:
            matches = sum(1 for word in source_words if len(word) > 2 and word in evidence_lower)
            source_present = matches >= len(source_words) * 0.7

    if not target_present:
        target_words = target_lower.split()
        if len(target_words) > 1:
            matches = sum(1 for word in target_words if len(word) > 2 and word in evidence_lower)
            target_present = matches >= len(target_words) * 0.7

    # Build reason if still not valid
    if not source_present and not target_present:
        return False, f"Both source '{rel.source}' and target '{rel.target}' not found in evidence"
    elif not source_present:
        return False, f"Source entity '{rel.source}' not found in evidence"
    elif not target_present:
        return False, f"Target entity '{rel.target}' not found in evidence"
...
BOOK_EXTRACTION_PROMPT = """Extract ALL meaningful relationships from this book segment. Your goal is to capture DATA, INFORMATION, KNOWLEDGE, and WISDOM.

## 🎯 ABSOLUTE REQUIREMENT: ENTITY PRESENCE ##

⚠️ CRITICAL: You MUST ONLY extract entities that ACTUALLY APPEAR in the text segment below.
- If an entity name is not in the text, DO NOT extract it
- Do not infer or assume entity names from context
- Both source AND target must appear in the text

Example:
✅ GOOD: Text says "Aaron William Perry" → Extract "Aaron William Perry"
❌ BAD: Text says "Copyright © 2018 Aaron William Perry" → DO NOT extract "Soil Stewardship Handbook" (not in text)

## 📚 WHAT TO EXTRACT ##

### Level 1: DATA (Facts)
- Names: Full names as they appear
- Dates: Exact dates, years
- Numbers: With context (10% of what?)
- Places: Complete location names

### Level 2: INFORMATION (Relationships)
- Authorship: X authored Y, X wrote Y
- Attribution: X said Y, X stated Y
- Definitions: X is defined as Y, X means Y
- Composition: X contains Y, X consists of Y

### Level 3: KNOWLEDGE (Causation & Processes)
- Causation: X causes Y, X leads to Y, X results in Y
- Processes: X involves Y, X requires Y, X produces Y
...
- Principles: Core truths, philosophical insights
- Recommendations: X should do Y, Choose X, X is recommended
- Wisdom: "What we do to soil, we do to ourselves" type statements

## ✅ EXTRACTION RULES ##

### 1. Extract COMPLETE entities with all qualifiers

DO:
✅ "organic biodegradable kitchen scraps" NOT just "kitchen scraps"
✅ "soil carbon content" NOT just "soil"
✅ "global soil carbon increase" NOT just "soil increase"
✅ "fossil fuel emissions" NOT just "emissions"
✅ "personal relationship with soil" NOT just "relationship"

### 2. Handle quantitative claims with FULL context

When you see numbers/percentages:

❌ BAD: soil → is increased by → 10%
✅ GOOD: global soil carbon content → can increase by → 10%

❌ BAD: carbon → equals → 3 gigatons
✅ GOOD: excess atmospheric carbon → equals → 3 gigatons

Always include WHAT is being measured and SCOPE (global, local, annual, etc.)

### 3. Preserve ALL important qualifiers

Include scope: global, worldwide, local, regional
Include type: organic, natural, chemical, synthetic
Include time: annual, daily, long-term, short-term
Include quality: active, stable, fresh, living

Examples:
✅ "active living soil" (includes both qualifiers)
✅ "annual fossil fuel emissions" (includes time scope)
✅ "natural soil building cycle" (includes qualifier)

### 4. Extract quotes and attributions completely

When someone said something:
- Source: Person's full name
- Target: Complete quote or principle stated
- Relationship: "stated", "said", "believes", "teaches"

Example:
Text: "What we do to the soil, we do to ourselves." —Vandana Shiva
✅ Extract: Vandana Shiva → stated principle → "What we do to the soil, we do to ourselves"

### 5. Extract wisdom, principles, recommendations

Look for:
- Should/must statements: "We must become expert stewards"
- Core truths: "Soil stewardship is sacred work"
- Principles: "Connection with soil unlocks growth"
- Recommendations: "Choose organic products"

### 6. Evidence text MUST be substantial

- Include enough context to understand the relationship
- Minimum 50 characters, prefer 100-300 characters
- Must contain BOTH source and target entities
- Include surrounding context

❌ BAD evidence: "...soil is essential..."
✅ GOOD evidence: "Soil is not just sustenance. Soil is essential to our survival and provides the foundation of all human life on Earth."

## 🚫 WHAT NOT TO DO ##

❌ Do NOT infer entities not in the text
❌ Do NOT extract generic entities: "it", "this", "that", "thing"
❌ Do NOT use shortened forms if full form appears: "soil carbon" → use "soil carbon content" if that's what the text says
❌ Do NOT lose qualifiers from the text
❌ Do NOT extract relationships where entities don't appear in evidence

## 📝 OUTPUT FORMAT ##

For each relationship provide:
- source: Complete source entity AS IT APPEARS in text (with all qualifiers)
- relationship: Clear, descriptive relationship type
- target: Complete target entity AS IT APPEARS in text (with all qualifiers)
- evidence_text: Substantial quote (100-300 chars) containing BOTH entities

## 📖 BOOK SEGMENT TO EXTRACT FROM ##

{text}

## ⚡ EXTRACT NOW ##

Extract ALL meaningful relationships following the rules above. Be COMPREHENSIVE but ACCURATE - only extract what's actually in the text."""
...
def extract_knowledge_graph_from_book(book_title: str,
    ...
    """
    ✨ IMPROVED: Three-stage book extraction with COMPLETE validation

    Extract → Type Validate → Score → Geographic Validate → Pattern Priors
    ...
    """
    logger.info(f"🚀 Starting IMPROVED v3.2.2 book extraction: {book_title}")
    ...
    # ========================================================================
    # ✨ ENTITY PRESENCE VALIDATION (NEW - FILTER OUT HALLUCINATED ENTITIES)
    # ========================================================================
    if not pass2_checkpoint:
        logger.info("🔍 ENTITY PRESENCE VALIDATION: Checking entities appear in evidence...")

        valid_presence = []
        entity_not_found = 0

        for candidate in all_candidates:
            is_valid, reason = validate_entity_presence(candidate)

            if not is_valid:
                entity_not_found += 1
                if candidate.flags is None:
                    candidate.flags = {}
                candidate.flags["ENTITY_NOT_IN_EVIDENCE"] = True
                candidate.flags["skip_reason"] = reason
                # Skip relationships where entities don't appear in evidence
                continue

            valid_presence.append(candidate)

        logger.info(f"✅ ENTITY PRESENCE VALIDATION COMPLETE:")
        logger.info(f"   - Valid (entities in evidence): {len(valid_presence)}")
        logger.info(f"   - Invalid (entities not found): {entity_not_found}")
        logger.info(f"   - Validation rate: {len(valid_presence)/len(all_candidates)*100:.1f}%")

        # Update all_candidates to only include valid ones
        all_candidates = valid_presence
    ...
    results = {
        ...
        'entity_presence_valid': len(valid_presence) if 'valid_presence' in locals() else len(all_candidates),
        'entity_not_found_filtered': entity_not_found if 'entity_not_found' in locals() else 0,
    ...
    logger.info(f"  - Entity presence validation: {results['entity_presence_valid']} valid ({results['entity_not_found_filtered']} filtered)")
...
def main():
    """Extract knowledge graph from Soil Stewardship Handbook with FULL validation"""
    logger.info("="*80)
    logger.info("🚀 IMPROVED KNOWLEDGE GRAPH EXTRACTION v3.2.2 - BOOK")
    logger.info("="*80)
    logger.info("")
    logger.info("✨ NEW FEATURES:")
    logger.info("  ✅ Type validation with SHACL-lite domain/range checking")
    logger.info("  ✅ Geographic validation with confidence penalties")
    logger.info("  ✅ Pattern priors from existing graph analysis")
    logger.info("  ✅ Complete validator integration")
    logger.info("")
    ...
    run_id = f"book_soil_handbook_improved_v3_2_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # Save results
    output_path = OUTPUT_DIR / f"{book_title.replace(' ', '_').lower()}_improved_v3_2_2.json"
    ...
    logger.info("")
    logger.info("="*80)
    logger.info("✨ IMPROVED BOOK EXTRACTION COMPLETE")
    logger.info("="*80)
