#!/usr/bin/env python3
"""
Knowledge Graph Extraction v6 - WITH REFLECTOR-DRIVEN IMPROVEMENTS

✨ V6 NEW FEATURES (Based on ACE Reflector Analysis):
✅ CRITICAL: POS-tagging for intelligent list splitting (fixes 32 errors)
✅ CRITICAL: Endorsement detection in bibliographic parser (fixes critical error)
✅ HIGH: Generic pronoun handler (fixes 8 errors)
✅ HIGH: Expanded vague entity patterns (fixes 20 errors)
✅ HIGH: Predicate normalizer module (fixes 12 errors)
✅ MEDIUM: Larger pronoun resolution window (fixes 7 errors)

GOAL: Reduce quality issues from 14.7% (V5) to <7% (V6)

Architecture: V5 base + 6 major improvements from Reflector analysis
...
OUTPUT_DIR = PLAYBOOK_DIR / "output" / "v6"
...
def _default_extraction_metadata():
    return {
        ...
        "extractor_version": "2025.10.12_v6",
        "content_type": "book",
        "run_id": None,
        "extracted_at": None,
        "batch_id": None
...
class BibliographicCitationParser:
    ...
    def __init__(self):
        ...
        # ✨ NEW: Endorsement detection patterns
        self.endorsement_patterns = [
            r'PRAISE FOR',
            r'(?:excellent|important|delightful|wonderful)\s+(?:tool|book|handbook|manual)',
            r'delighted to see',
            r'highly recommend',
...
class PronounResolver:
    ...
    def find_antecedent(self, pronoun: str, page_num: int, evidence_text: str) -> Optional[str]:
        ...
        # ✨ NEW: Increased window size from 500 to 1000
        ...
        # Collective pronouns
            ...
            # ✨ NEW: Look for cultural/national references (my people, our country)
            cultural_refs = re.findall(r'\b(my|our)\s+(people|country|nation|culture|heritage)\b', context, re.IGNORECASE)
            if cultural_refs:
                return cultural_refs[-1][1]
...
def pass_2_5_quality_post_processing(
    ...
    """
    ✨ V6 Pass 2.5: IMPROVED Quality Post-Processing Pipeline

    Now with 6 major improvements from Reflector analysis:
    1. Endorsement detection in bibliographic parser
    2. POS tagging for intelligent list splitting
    3. Generic pronoun handler
    4. Expanded vague entity patterns
    5. NEW: Predicate normalizer
    6. Larger pronoun resolution window
    """
    logger.info("🎨 PASS 2.5: V6 Improved Quality Post-Processing...")
    ...
    stats = {
        ...
        'endorsements_detected': 0,  # ✨ NEW
        'pronouns_resolved': 0,
        'generic_pronouns_resolved': 0,  # ✨ NEW
        'pronouns_unresolved': 0,
        'entities_enriched': 0,
        'entities_vague': 0,
        'lists_split': 0,
        'adjective_series_preserved': 0,  # ✨ NEW
        'predicates_normalized': 0,  # ✨ NEW
        'titles_incomplete': 0,
    ...
    # 1. Bibliographic Citation Parser (with endorsement detection)
    logger.info("  1/8: Bibliographic citation parsing + endorsement detection...")
    ...
    logger.info("  2/8: Title completeness validation...")
    ...
    logger.info("  3/8: Predicate validation...")
    ...
    # 4. ✨ NEW: Predicate Normalizer
    logger.info("  4/8: Predicate normalization...")
    ...
    # 5. Pronoun Resolver (with generic pronoun handling + larger window)
    logger.info("  5/8: Pronoun resolution (generic + anaphoric)...")
    ...
    # 6. Context Enricher (with expanded vague patterns)
    logger.info("  6/8: Context enrichment (expanded patterns)...")
    ...
    # 7. List Splitter (with POS tagging for adjective series)
    logger.info("  7/8: List splitting (POS-aware)...")
    ...
    # 8. Figurative Language Filter
    logger.info("  8/8: Figurative language detection...")
    ...
    logger.info(f"✅ PASS 2.5 V6 COMPLETE:")
    logger.info(f"   - Initial: {initial_count} relationships")
    logger.info(f"   - Authorship reversed: {stats['authorship_reversed']}, Endorsements detected: {stats['endorsements_detected']}")
    logger.info(f"   - Pronouns: {stats['pronouns_resolved']} anaphoric + {stats['generic_pronouns_resolved']} generic resolved ({stats['pronouns_unresolved']} unresolved)")
...
def extract_knowledge_graph_from_book_v6(book_title: str,
                                         pdf_path: Path,
                                         run_id: str,
                                         batch_size: int = 25) -> Dict[str, Any]:
    """
    ✨ V6 EXTRACTION SYSTEM WITH REFLECTOR-DRIVEN IMPROVEMENTS

    V5 (Pass 1 + Pass 2 + Pass 2.5) → ✨ IMPROVED V6 Pass 2.5 → Output

    GOAL: Reduce quality issues from 14.7% (V5) to <7% (V6)
    """
    logger.info(f"🚀 Starting V6 extraction with Reflector improvements: {book_title}")
    ...
    logger.info(f"📊 FINAL V6 RESULTS:")
...
def main():
    """Extract knowledge graph from Soil Stewardship Handbook with V6 system"""
    logger.info("="*80)
    logger.info("🚀 V6 KNOWLEDGE GRAPH EXTRACTION - WITH REFLECTOR-DRIVEN IMPROVEMENTS")
    logger.info("="*80)
    logger.info("")
    logger.info("✨ V6 IMPROVEMENTS (From ACE Reflector Analysis):")
    logger.info("  ✅ CRITICAL: POS tagging for intelligent list splitting (fixes 32 errors)")
    logger.info("  ✅ CRITICAL: Endorsement detection (fixes critical error)")
    logger.info("  ✅ HIGH: Generic pronoun handler (fixes 8 errors)")
    logger.info("  ✅ HIGH: Expanded vague entity patterns (fixes 20 errors)")
    logger.info("  ✅ HIGH: NEW Predicate normalizer module (fixes 12 errors)")
    logger.info("  ✅ MEDIUM: Larger pronoun resolution window (fixes 7 errors)")
    logger.info("")
    logger.info("GOAL: Reduce quality issues from 14.7% (V5) to <7% (V6)")
    logger.info("")
    ...
    run_id = f"book_soil_handbook_v6_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

    start_time = time.time()

    # Extract with V6 system
    results = extract_knowledge_graph_from_book_v6(
        book_title=book_title,
        pdf_path=pdf_path,
        run_id=run_id,
        batch_size=25
    ...
    # Save results
    output_path = OUTPUT_DIR / f"{book_title.replace(' ', '_').lower()}_v6.json"
    ...
    logger.info("")
    logger.info("="*80)
    logger.info("✨ V6 EXTRACTION COMPLETE")
    logger.info("="*80)
    logger.info(f"⏱️  Total time: {total_time/60:.1f} minutes")
    logger.info(f"📁 Results saved to: {output_path}")
    logger.info("")
    logger.info("NEXT STEPS:")
    logger.info("1. Compare V6 vs V5 quality metrics")
    logger.info("2. Run KG Reflector on V6 to measure improvements")
    logger.info("3. Continue ACE cycle if issues >5%")
    logger.info("="*80)
