#!/usr/bin/env python3
"""
Knowledge Graph Extraction V11.2.2 - Quality Fixes Based on Reflector Analysis

🎯 V11.2.2 FIXES (from V11.2.1):
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'kg_extraction_book_v11_2_2_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
OUTPUT_DIR = PLAYBOOK_DIR / "output" / "v11_2_2"
...
def postprocess_pass2_5(
    ...
    ✨ V11.2.2 FIX: Uses fixed modules (dedication parser, list splitter, predicate normalizer)
    ...
    # Get book pipeline (all 12 modules with V11.2.2 fixes)
...
def extract_knowledge_graph_v11_2_2(
    book_title: str,
    pdf_path: Path,
    ...
    """
    V11.2.2 knowledge graph extraction pipeline with quality fixes.
    ...
    5. Pass 2.5: Modular postprocessing with V11.2.2 fixed modules
    6. Return results with full metadata
    """
    logger.info(f"🚀 Starting V11.2.2 extraction: {book_title}")
    ...
    # Step 5: Pass 2.5 - Modular postprocessing with V11.2.2 fixes
    context = ProcessingContext(
        content_type='book',
    ...
    results = {
        'metadata': {
            ...
            'fixes_applied': [
                ...
                'V11.2.2: Fixed dedication parser - extract only proper names, filter fragments',
                'V11.2.2: Fixed ListSplitter - POS tagging to avoid splitting adverbs/verbs',
                'V11.2.2: Enhanced Predicate Normalizer - 173 → ~80 unique predicates'
            ]
        },
        'extraction_stats': {
    ...
    logger.info("")
    logger.info("📊 FINAL V11.2.2 RESULTS:")
    ...
    logger.info(f"  - ✨ V11.2.2 Pass 2.5 final: {len(final_relationships)}")
...
def main():
    """Extract knowledge graph from Soil Stewardship Handbook with V11.2.2 quality fixes"""
    logger.info("="*80)
    logger.info("🚀 V11.2.2 KNOWLEDGE GRAPH EXTRACTION - QUALITY FIXES")
    logger.info("="*80)
    logger.info("")
    logger.info("✨ V11.2.2 FIXES (based on Reflector analysis):")
    ...
    logger.info("  - V11.2.2 Target: B or better (<15% error rate)")
    ...
    run_id = f"book_soil_handbook_v11_2_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # Extract with V11.2.2 fixed system
    results = extract_knowledge_graph_v11_2_2(
        book_title=book_title,
        pdf_path=pdf_path,
        document_metadata=document_metadata,
        run_id=run_id,
    ...
    # Save results
    output_path = OUTPUT_DIR / f"{book_title.replace(' ', '_').lower()}_v11_2_2.json"
    ...
    logger.info("")
    logger.info("="*80)
    logger.info("✨ V11.2.2 EXTRACTION COMPLETE")
    logger.info("="*80)
    logger.info(f"⏱️  Total time: {total_time/60:.1f} minutes")
    logger.info(f"📁 Results saved to: {output_path}")
    logger.info("")
    logger.info("NEXT STEPS:")
    logger.info("1. Run KG Reflector on V11.2.2 to verify quality improvements")
    logger.info("2. Compare V11.2.2 vs V11.2.1 (C-) vs V10 vs V9 quality metrics")
    logger.info("3. Verify the 3 fixes worked (check module_flags in output)")
    logger.info("4. If quality reaches B grade, V11.2.2 becomes new baseline")
    logger.info("="*80)
