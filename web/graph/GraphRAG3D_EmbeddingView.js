...
class GraphRAG3DEmbeddingView {
    ...
    async fetchJsonWithFallback(paths, label = 'resource') {
        ...
        const cacheBuster = 'v=20251204i';

        for (const url of urls) {
            try {
                const bustUrl = url + (url.includes('?') ? '&' : '?') + cacheBuster;
                console.log(`Fetching ${label}: ${bustUrl}`);
    ...
    async fetchHierarchyData() {
        // Use original hierarchy file - it has correct structure
        // level_1.children contains parent refs to level_2 IDs
        // level_2.children contains entity names
        // Note: cache buster is added by fetchJsonWithFallback
        ...
        for (const { key, alt } of hierarchyLevels) {
            ...
            for (const [clusterId, cluster] of Object.entries(levelClusters)) {
                ...
                const entry = {
                    ...
                    summary_text: cluster.summary_text || cluster.summary || null
                };
    ...
    buildHierarchicalData() {
        ...
        const level2ById = new Map();
        for (const c of level2Clusters) {
            level2ById.set(c.id, c);
        }
        ...
        // - Only 3 level_1 clusters have 'children' that point to level_2 IDs
        // - These 3 mega-clusters contain all 73 level_2 sub-clusters
        // - Hierarchy: 3 mega-clusters (L3) → 73 sub-clusters (L2) → entities
        else if (level2Clusters.length > 0 && level1Clusters.length > 0) {
            console.log('Building hierarchy from mega-clusters');

            // Find level_1 clusters that have children pointing to level_2
            const megaClusters = level1Clusters.filter(c => (c.children || []).length > 0);
            console.log(`Found ${megaClusters.length} mega-clusters (level_1 with level_2 children)`);

            if (megaClusters.length > 0) {
                // Build hierarchy: mega-clusters (L3) → level_2 (L2)
                for (const megaCluster of megaClusters) {
                    // Debug: log raw cluster data
                    console.log(`  Raw mega-cluster:`, {
                        id: megaCluster.id,
                        title: megaCluster.title,
                        name: megaCluster.name,
                        childrenCount: (megaCluster.children || []).length
                    });

                    const topNode = {
                        name: megaCluster.title || megaCluster.name || megaCluster.id,
                        id: megaCluster.id,
                        level: 3,  // L3 = top visible level
                        color: '#FF4444',
                        value: megaCluster.entities?.length || 100,
                        children: [],
                        summary_text: megaCluster.summary_text || megaCluster.summary || null,
                    };

                    // Add level_2 children (the sub-clusters)
                    const childIds = megaCluster.children || [];
                    console.log(`  Mega "${topNode.name}": ${childIds.length} level_2 children`);

                    for (const childId of childIds) {
                        const l2Cluster = level2ById.get(childId);
                        if (!l2Cluster) continue;

                        // level_2.children contains entity names directly
                        const entityNames = l2Cluster.children || [];
                        const subNode = {
                            name: l2Cluster.title || l2Cluster.name || childId,
                            id: childId,
                            level: 2,  // L2 = revealed on hover
                            color: '#FFCC00',
                            value: entityNames.length || 1,
                            entityIds: entityNames,  // These are entity names
                            summary_text: l2Cluster.summary_text || l2Cluster.summary || null,
                        };
                ...
                console.log(`Built ${root.children.length} top-level clusters (mega-clusters)`);
            } else {
                // Fallback: use level_2 clusters directly as flat list
                console.log('No mega-clusters found, using level_2 as flat list');
                for (const l2Cluster of level2Clusters.slice(0, 100)) {
                    const entityNames = l2Cluster.children || [];
                    const node = {
                        name: l2Cluster.title || l2Cluster.name || l2Cluster.id,
                        id: l2Cluster.id,
                        level: 3,
                        color: '#FF4444',
                        value: entityNames.length || 1,
    ...
    showVoronoiClusterInfo(cell) {
        ...
        const clusterSummary = cell.data.summary_text || cell.data.summary || null;
        ...
        // Build hierarchy HTML with clickable parent links
        let hierarchyHtml = '';
        // Store cell references for hierarchy navigation
        const cellsByLevel = {};
        let currentCell = cell;
        while (currentCell) {
            if (currentCell.level !== undefined) {
                cellsByLevel[currentCell.level] = currentCell;
            }
        ...
        if (hierarchyPath.length > 0) {
            hierarchyHtml = `
                <div style="margin-top: 16px; padding: 12px; background: rgba(100, 200, 255, 0.05); border: 1px solid rgba(100, 200, 255, 0.15); border-radius: 10px;">
                    ...
                    ${hierarchyPath.map((h, idx) => {
                        const isCurrentLevel = h.level === cell.level;
                        const isClickable = !isCurrentLevel && h.level > 0;
                        return `
                        <div style="display: flex; align-items: center; margin: 6px 0; padding-left: ${idx * 12}px;">
                            ...
                            <span class="${isClickable ? 'hierarchy-link' : ''}"
                                  data-level="${h.level}"
                                  style="color: ${isCurrentLevel ? '#64b5f6' : '#ccc'}; font-size: 13px; ${isCurrentLevel ? 'font-weight: 600;' : ''} ${isClickable ? 'cursor: pointer; text-decoration: underline; text-decoration-style: dotted;' : ''}"
                                  ${isClickable ? `onmouseover="this.style.color='#64b5f6'" onmouseout="this.style.color='#ccc'"` : ''}
                            >${h.name}</span>
                        </div>
                    `;}).join('')}
                </div>
            ...
            // Store cell references for click handling
            this._hierarchyCells = cellsByLevel;
        }

        // Get children info if available - show clickable list for L3 clusters
        let childrenHtml = '';
        if (cell.children && cell.children.length > 0) {
            ...
            // For L3 clusters, show clickable list of nested L2 clusters
            if (cell.level === 3) {
                // Sort children by entity count (largest first)
                const sortedChildren = [...cell.children].sort((a, b) => {
                    const aCount = a.data?.entityIds?.length || a.data?.entities?.length || 0;
                    const bCount = b.data?.entityIds?.length || b.data?.entities?.length || 0;
                    return bCount - aCount;
                });

                // Store children for click handling
                this._nestedClusters = {};
                sortedChildren.forEach((child, idx) => {
                    this._nestedClusters[idx] = child;
                });

                childrenHtml = `
                    <div style="margin-top: 16px; padding: 12px; background: rgba(255, 204, 0, 0.08); border: 1px solid rgba(255, 204, 0, 0.2); border-radius: 10px;">
                        <div style="font-size: 12px; color: #FFCC00; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 12px; display: flex; justify-content: space-between; align-items: center;">
                            <span>Nested Communities</span>
                            <span style="color: #ccc; font-size: 11px;">${childCount} clusters</span>
                        </div>
                        ${childCount > 10 ? `
                            <div style="margin-bottom: 12px;">
                                <input type="text" id="voronoi-cluster-search" placeholder="Search clusters..."
                                       style="width: 100%; padding: 8px 12px; border: 1px solid rgba(255, 204, 0, 0.3); border-radius: 6px; background: rgba(0,0,0,0.3); color: #fff; font-size: 12px; outline: none;"
                                       onfocus="this.style.borderColor='rgba(255, 204, 0, 0.6)'"
                                       onblur="this.style.borderColor='rgba(255, 204, 0, 0.3)'">
                            </div>
                        ` : ''}
                        <div id="voronoi-cluster-list" style="max-height: 350px; overflow-y: auto;">
                            ${sortedChildren.map((child, idx) => {
                                const childName = child.data?.name || child.data?.id || 'Unknown';
                                const childEntityCount = child.data?.entityIds?.length || child.data?.entities?.length || 0;
                                return `
                                    <div class="nested-cluster-item" data-cluster-idx="${idx}" data-cluster-name="${childName.toLowerCase()}"
                                         style="padding: 8px 10px; margin: 3px 0; background: rgba(255,255,255,0.03); border-radius: 6px; cursor: pointer; display: flex; justify-content: space-between; align-items: center; transition: background 0.2s;"
                                         onmouseover="this.style.background='rgba(255,204,0,0.15)'"
                                         onmouseout="this.style.background='rgba(255,255,255,0.03)'">
                                        <span style="color: #ccc; font-size: 13px;">${childName}</span>
                                        <span style="color: #888; font-size: 11px;">${childEntityCount} entities</span>
                                    </div>
                                `;
                            }).join('')}
                        </div>
        ...
        // Build entity list HTML for leaf clusters (L1 or L2 depending on hierarchy)
        // In 2-level voronoi: L2 is the leaf level with entities
        // In 3-level hierarchy: L1 is the leaf level
        let entitiesHtml = '';
        const isLeafCluster = (cell.level === 1 || cell.level === 2) && entityIds.length > 0;
        ...
        if (isLeafCluster) {
            ...
            for (const entityId of entityIds) {
                ...
                if (entity) {
                    ...
                    entitiesByType[type].push(entityData);
            ...
            // Store entities for search filtering
            this._currentClusterEntities = allEntities;
            this._currentEntitiesByType = entitiesByType;
            ...
            entitiesHtml = `
                <div style="margin-top: 16px; padding: 12px; background: rgba(136, 255, 136, 0.05); border: 1px solid rgba(136, 255, 136, 0.2); border-radius: 10px;">
                    ...
                    ${entityIds.length > 10 ? `
                        <div style="margin-bottom: 12px;">
                            <input type="text" id="voronoi-entity-search" placeholder="Search entities..."
                                   style="width: 100%; padding: 8px 12px; border: 1px solid rgba(100, 180, 246, 0.3); border-radius: 6px; background: rgba(0,0,0,0.3); color: #fff; font-size: 12px; outline: none;"
                                   onfocus="this.style.borderColor='rgba(100, 180, 246, 0.6)'"
                                   onblur="this.style.borderColor='rgba(100, 180, 246, 0.3)'">
                        </div>
                    ` : ''}
                    <div id="voronoi-entity-list" style="max-height: 350px; overflow-y: auto;">
                        ${sortedTypes.map(type => `
                            <div class="entity-type-group" data-type="${type}" style="margin-bottom: 12px;">
                                <div style="font-size: 11px; color: ${typeColors[type] || '#95A5A6'}; font-weight: 600; margin-bottom: 6px; display: flex; align-items: center;">
                                    ...
                                    ${type} (<span class="type-count">${entitiesByType[type].length}</span>)
                                </div>
                                <div class="entity-items">
                                    ${entitiesByType[type].map(e => `
                                        <div class="entity-item" data-entity-id="${e.id}" data-entity-name="${e.name.toLowerCase()}"
        ...
        if (clusterSummary) {
            summaryHtml = `
                <div style="margin-top: 16px; padding: 12px; background: rgba(100, 180, 246, 0.08); border: 1px solid rgba(100, 180, 246, 0.2); border-radius: 10px;">
                    <div style="font-size: 12px; color: #64b5f6; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px;">Summary</div>
                    <div style="color: #d0d0d0; font-size: 13px; line-height: 1.5;">${clusterSummary}</div>
                </div>
            `;
        }
        ...
        voronoiInfo.innerHTML = `
            <div class="cluster-info">
                ...
                ${hierarchyHtml}
                ${entitiesHtml}
                ${cell.level !== 1 && !isLeafCluster ? `<div style="margin-top: 12px; font-size: 12px; color: #7a8ca8;">
                    Click on nested cells to explore deeper levels
                </div>` : ''}
            </div>
        `;

        // Add click handlers for entity items (works for both L1 and L2 leaf clusters)
        if (isLeafCluster) {
            ...
            entityItems.forEach(item => {
                item.addEventListener('click', () => {
                    ...
                    if (entityId) {
                        // Show entity info in the same panel (like semantic view)
                        this.showEntityInfo(entityId, true);
                    }
                });
            });
            ...
            const searchInput = voronoiInfo.querySelector('#voronoi-entity-search');
            if (searchInput) {
                searchInput.addEventListener('input', (e) => {
                    const query = e.target.value.toLowerCase().trim();
                    const entityList = voronoiInfo.querySelector('#voronoi-entity-list');
                    const typeGroups = entityList.querySelectorAll('.entity-type-group');
                    ...
                    typeGroups.forEach(group => {
                        ...
                        items.forEach(item => {
                            const name = item.getAttribute('data-entity-name') || '';
                            const matches = !query || name.includes(query);
                            item.style.display = matches ? 'block' : 'none';
                            if (matches) visibleCount++;
                        });

                        // Update type count and hide empty groups
                        const countSpan = group.querySelector('.type-count');
                        if (countSpan) {
        ...
        // Add click handlers for hierarchy links (parent clusters)
        const hierarchyLinks = voronoiInfo.querySelectorAll('.hierarchy-link');
        hierarchyLinks.forEach(link => {
            link.addEventListener('click', () => {
                const level = parseInt(link.getAttribute('data-level'));
                const parentCell = this._hierarchyCells?.[level];
                if (parentCell) {
                    this.showVoronoiClusterInfo(parentCell);
                }
            });
        });

        // Add click handlers for nested cluster items (L3 → L2 navigation)
        const nestedClusterItems = voronoiInfo.querySelectorAll('.nested-cluster-item');
        nestedClusterItems.forEach(item => {
            item.addEventListener('click', () => {
                const clusterIdx = parseInt(item.getAttribute('data-cluster-idx'));
                const childCell = this._nestedClusters?.[clusterIdx];
                if (childCell) {
                    this.showVoronoiClusterInfo(childCell);
                }
            });
        });

        // Add search functionality for nested clusters
        const clusterSearchInput = voronoiInfo.querySelector('#voronoi-cluster-search');
        if (clusterSearchInput) {
            clusterSearchInput.addEventListener('input', (e) => {
                const query = e.target.value.toLowerCase().trim();
                const clusterList = voronoiInfo.querySelector('#voronoi-cluster-list');
                const items = clusterList.querySelectorAll('.nested-cluster-item');

                items.forEach(item => {
                    const name = item.getAttribute('data-cluster-name') || '';
                    const matches = !query || name.includes(query);
    ...
    renderVoronoiTreemap(svg, g, width, height) {
        ...
        cells
            ...
            .on('click', (event, d) => {
                ...
                // Show cluster info in the side panel (entities for L2, sub-clusters for L3)
        ...
        // Debug: Log sample L3 and their L2 children
        const sampleL3 = allCells.filter(c => c.level === 3).slice(0, 3);
        sampleL3.forEach(l3 => {
            const l2Kids = allCells.filter(c => c.level === 2 && c.parent === l3);
            console.log(`L3 "${l3.data?.name}" has ${l2Kids.length} L2 children (from l3.children: ${l3.children?.length})`);
        });
    }
