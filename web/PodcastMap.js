...
class PodcastMapVisualization {
    ...
    showEpisodeTrajectory(episodeId) {
        ...
        // Highlight episode points - dulled state when not playing
        this.circles
            .transition()
            .duration(this.transitionDuration)
            .attr('r', d => d.episode_id === episodeId ? 4 : 3)
            .attr('opacity', d => {
                if (d.episode_id === episodeId) return 0.9;
                return 0.15; // Very light for non-selected episodes
            });
    }
    ...
    async loadAudioForEpisode(episodeId) {
        ...
        try {
            const episodeNumber = episodeId.split(' ')[0].replace('Episode', '').trim();
    ...
    highlightActiveChunk(activeChunk) {
        ...
        this.circles
            ...
            .duration(200)
            .attr('r', d => {
                if (d.id === activeChunk.id) return 8; // Active chunk - largest
                if (d.episode_id === this.selectedEpisode) {
                    if (prevChunk && d.id === prevChunk.id) return 6; // Previous chunk
                    if (nextChunk && d.id === nextChunk.id) return 6; // Next chunk
                    return 4; // Other episode chunks
                }
            ...
            .attr('opacity', d => {
                ...
                return 0.1; // Very light for all other points
            });
